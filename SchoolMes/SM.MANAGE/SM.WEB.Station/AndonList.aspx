<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AndonList.aspx.cs" Inherits="SM.WEB.Station.AndonList" %>
...
<body class="linen">
    <form id="form1" runat="server">
        <div>
            <div class="div">
                ...
                <div style="height: 170px;">
                    <span class="span">线体</span>
                    <select name="domain" class="select full-width margin-top" id="line" onchange="GetStation(this)">
                        <option value="" class="option">请选择线体</option>
                    </select>
                    <span class="span">Andon</span>
                    <select name="domain" class="select full-width margin-top" id="station">
                        <option value="" class="option">请选择Andon</option>
                    </select>
                </div>
        ...
        <script type="text/javascript">
            ...
            function GetLine() {
                $.post('/Controller/Andon/getELine.ashx', function (data) {
            ...
            function GetStation(obj) {
                var lineid = $(obj).val();
                $.post('/Controller/Andon/getAndonByELineId.ashx', { elineid: lineid }, function (data) {
                    var ohtml = '<option value="" class="option">请选择Andon</option>';
                    if (data.length > 0) {
                        for (var i = 0; i < data.length; i++) {
                            ohtml += '<option value="' + data[i].ID + '" class="option">' + data[i].AndonName + '</option>';
                        }
                    }
                    $("#station").html(ohtml);
                }, "json");
            ...
            function CheckInput() {
                ...
                if (stationid !== "") {
                    $.post('/Controller/Andon/getAndonById.ashx', { id: stationid }, function (data) {
                        if (data.length > 0) {
                            window.location.href = "Andon.aspx?Id=" + data[0].ID ;
                        }
                    }, "json");
                }
                else {
