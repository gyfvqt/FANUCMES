<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Station.aspx.cs" Inherits="SM.WEB.Station.Station" %>
...
<body class="linen">
    <form id="form1" runat="server">
        ...
        <script type="text/javascript">
            ...
            function GetLine() {
                $.post('/Controller/getLine.ashx', function (data) {
                    var ohtml = '<option value="" class="option">请选择线体</option>';
                    if (data.length > 0) {
                        for (var i = 0; i < data.length; i++) {
                            ohtml += '<option value="' + data[i].ID + '" class="option">' + data[i].LineName + '</option>';
                        }
                    }
                    $("#line").html(ohtml);
                }, "json");
            ...
            function CheckInput() {
                ...
                if (stationid !== "") {
                    $.post('/Controller/getStationById.ashx', { stationid: stationid }, function (data) {
                        if (data.length > 0) {
                            if (data[0].StationType == "PC") {
                                window.location.href = "PCStationView.aspx?stationid="+data[0].ID+"&stationcode="+data[0].StationCode;
                            }
                            else if (data[0].StationType == "PLC") {
                                window.location.href = "PLCStationView.aspx?stationid="+data[0].ID+"&stationcode="+data[0].StationCode;
                            }
