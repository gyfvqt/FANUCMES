...
namespace SM.WEB.Controller
{
    ...
    /// userSearch 的摘要说明
    /// </summary>
    public class userSearch : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (Email.Trim() != "")
                {
                    sqlwhere += " AND a.Email like N'%" + Email.Trim() + "%'";
                }

                if (PhoneNumber.Trim() != "")
                {
                    sqlwhere += " AND a.PhoneNumber like N'%" + PhoneNumber.Trim() + "%'";
                }

                string sqlCount = string.Format(@"select count(1) from  [UserInfo](nolock)  a
...
FROM    ( SELECT TOP ( {0} * {1} )
                    ...
                    a.*,b.RoleName
            FROM  UserInfo(nolock)  a   join UserRole b on a.RoleId=b.ID        
where 1=1  {2}  
        ) AS temp
