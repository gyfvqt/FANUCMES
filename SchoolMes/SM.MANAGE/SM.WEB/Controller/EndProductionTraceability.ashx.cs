...
{
    /// <summary>
    /// EndProductionTraceability 的摘要说明
    /// </summary>
    public class EndProductionTraceability : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                string ParkSn = HttpContext.Current.Request.Params["parkSn"];
                ...
                if (ParkSn.Trim() != "")
                {
                    sqlwhere += " AND a.ParkSn like N'%" + ParkSn.Trim() + "%'";
                }
                ...
                string sqlCount = string.Format(@"select count(1) from  ProductTraceabilityInfo(nolock) a left join EndProduct(nolock) b on a.ProductId=b.ID 
...
FROM    ( SELECT TOP ( {0} * {1} )
                    ...
                    a.StationName,a.ParkSn,a.TransitTime,b.*
              FROM ProductTraceabilityInfo(nolock) a left join EndProduct(nolock) b on a.ProductId=b.ID 
where 1=1  {2}  
        ) AS temp
