...
{
    /// <summary>
    /// warehouseDelete 的摘要说明
    /// </summary>
    public class warehouseDelete : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
                ...
                if (ID.Trim() != "")
                {
                    ...
                    if (list.Length > 0)
                    {
                        for (int i = 0; i < list.Length; i++)
                        {
                            if (list[i].Trim() != "")
                            {
                                sql += string.Format(@"delete from Warehouse  where ID =N'{0}';", list[i]);
                            }
                        }
