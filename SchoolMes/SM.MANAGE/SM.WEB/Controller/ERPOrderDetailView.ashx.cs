...
{
    /// <summary>
    /// ERPOrderDetailView 的摘要说明
    /// </summary>
    public class ERPOrderDetailView : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (ID.Trim() != "")
                {
                    sqlwhere += " AND ERPID = N'" + ID.Trim() + "'";
                }                
                string sqlSearch = string.Format(@"select * from ERPOrderDetails(nolock) where 1=1 {0}",  sqlwhere);
