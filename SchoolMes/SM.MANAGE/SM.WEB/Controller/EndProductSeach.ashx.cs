...
{
    /// <summary>
    /// EndProductSeach 的摘要说明
    /// </summary>
    public class EndProductSeach : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (ID.Trim() != "")
                {
                    sqlwhere += " AND ERPDetailId = N'" + ID.Trim() + "'";
                }
                string sqlSearch = string.Format(@"select * from EndProduct(nolock) where 1=1 {0}", sqlwhere);
