...
{
    /// <summary>
    /// productionDelete 的摘要说明
    /// </summary>
    public class productionDelete : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
                ...
                if (ID.Trim() != "")
                {
                    ...
                    if (list.Length > 0)
                    {
                        for (int i = 0; i < list.Length; i++)
                        {
                            if (list[i].Trim() != "")
                            {
                                sql += string.Format(@"delete from ProductionInfo  where ID =N'{0}';", list[i]);
                            }
                        }
