...
{
    /// <summary>
    /// warehouseSearch 的摘要说明
    /// </summary>
    public class warehouseSearch : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (WarehouseNo.Trim() != "")
                {
                    sqlwhere += " AND a.WarehouseNo like N'%" + WarehouseNo.Trim() + "%'";
                }
                if (WarehouseName.Trim() != "")
                {
                    sqlwhere += " AND a.WarehouseName like N'%" + WarehouseName.Trim() + "%'";
                }

                if (WarehouseType.Trim() != "")
                {
                    sqlwhere += " AND a.WarehouseType like N'%" + WarehouseType.Trim() + "%'";
                }

                string sqlCount = string.Format(@"select count(1) from  [Warehouse](nolock)  a
