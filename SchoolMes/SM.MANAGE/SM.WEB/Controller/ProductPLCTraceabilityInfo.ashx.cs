...
{
    /// <summary>
    /// ProductPLCTraceabilityInfo 的摘要说明
    /// </summary>
    public class ProductPLCTraceabilityInfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (ID.Trim() != "")
                {
                    sqlwhere += " AND ProductId = N'" + ID.Trim() + "'";
                }
                string sqlSearch = string.Format(@"select * from ProductPLCTraceabilityInfo(nolock) where 1=1 {0}", sqlwhere);
