...
namespace SM.WEB.Controller
{
    ...
    /// LineSearch 的摘要说明
    /// </summary>
    public class LineSearch : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (LineId.Trim() != "")
                {
                    sqlwhere += " AND a.LineId like N'%" + LineId.Trim() + "%'";
                }
                if (LineName.Trim() != "")
                {
                    sqlwhere += " AND a.LineName like N'%" + LineName.Trim() + "%'";
                }

                if (LineType.Trim() != "")
                {
                    sqlwhere += " AND a.LineType like N'%" + LineType.Trim() + "%'";
                }

                string sqlCount = string.Format(@"select count(1) from  [LineInfo](nolock)  a
