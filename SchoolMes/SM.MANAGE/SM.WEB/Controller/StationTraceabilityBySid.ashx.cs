...
{
    /// <summary>
    /// StationTraceabilityBySid 的摘要说明
    /// </summary>
    public class StationTraceabilityBySid : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (ID.Trim() != "")
                {
                    sqlwhere += " AND PCStationId = N'" + ID.Trim() + "'";
                }
                string sqlSearch = string.Format(@"select * from PCTraceability(nolock) where 1=1 {0}", sqlwhere);
