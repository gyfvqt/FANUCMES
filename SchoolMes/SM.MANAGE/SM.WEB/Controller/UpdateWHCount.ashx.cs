...
namespace SM.WEB.Controller
{
    ...
    /// UpdateWHCount 的摘要说明
    /// </summary>
    public class UpdateWHCount : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
                    ...
                    if (list.Length > 0)
                    {
                        for (int i = 0; i < list.Length; i++)
                        {
                            if (list[i].Trim() != "")
                            {
                                ...
                                sqlwms += string.Format(@"update MaterialCallPoint set WHCount=N'{0}' where ID=N'{1}' ;", list2[1], list2[0]);
                            }
                        }
