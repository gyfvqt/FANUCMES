...
{
    /// <summary>
    /// PLCTemplateInfoDetailSearch 的摘要说明
    /// </summary>
    public class PLCTemplateInfoDetailSearch : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (ID!= null)
                {
                    sqlwhere += " AND PLCTemplateID = N'" + ID.Trim() + "'";
                }
                string sqlSearch = string.Format(@"select * from PLCTemplateInfoDetail(nolock) where 1=1 {0}", sqlwhere);
