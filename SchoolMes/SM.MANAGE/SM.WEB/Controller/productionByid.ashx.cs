...
{
    /// <summary>
    /// productionByid 的摘要说明
    /// </summary>
    public class productionByid : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (ID.Trim() != "")
                {
                    sqlwhere += "  AND ID=" + ID.Trim();
                }

                string sqlSearch = string.Format(@"SELECT * from ProductionInfo where 1=1  {0} ", sqlwhere);
