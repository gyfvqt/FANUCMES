<%@ Page Title="" Language="C#" MasterPageFile="~/PageM.Master" AutoEventWireup="true" CodeBehind="EndProductList.aspx.cs" Inherits="SM.WEB.Views.OrderManagement.EndProductList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:Label runat="server" ID="lblPage" Visible="false" Text="成品查询"></asp:Label>
    <section id="main" role="main">
        ...
        <div style="height: 28px;">
            <span style="font-weight: bold; text-transform: uppercase; font-size: 16px; color: #444; float: left; margin-left: 12px; margin-top: 10px;">当前页面:订单管理\成品查询</span>
        </div>
                        ...
                        <div class="with-small-padding">
                            <div class="RadAjaxPanel" style="display: block; height: 100px; overflow-y: auto;">
                                <div>
                                    ...
                                    <input id="txtEndProductSN" type="text" class="k-textbox" style="margin: 0 5px 5px 0; height: 26px; width: 120px; float: left;" />
                                    ...
                                    <div style="padding-bottom: .3em; font-weight: bold; float: left; text-transform: uppercase; font-size: 14px; color: #444;">产品图号:</div>
                                    <input id="txtProductionSheet" type="text" class="k-textbox" style="margin: 0 5px 5px 0; height: 26px; width: 120px; float: left;" />
                                    <div style="padding-bottom: .3em; font-weight: bold; float: left; text-transform: uppercase; font-size: 14px; color: #444;">订单编号:</div>
                                    <input id="txtERPDetailCode" type="text" class="k-textbox" style="margin: 0 5px 5px 0; height: 26px; width: 120px; float: left;" />
                                    <br />
                                    
                                    <div style="padding-bottom: .3em; font-weight: bold; float: left; text-transform: uppercase; font-size: 14px; color: #444;">
                                        状态:
                                    <select style="margin: 0 5px 5px 0; border: 1px solid #ccc; height: 28px; width: 120px;" id="SelectStatus">
                                        ...
                                        过站时间 :  过点                                    
                                    <select style="margin: 0 5px 5px 0; border: 1px solid #ccc; height: 28px; width: 120px;" id="sStationName">
                                        <option value="" selected="selected">请选择</option>
                                    </select>
                                        从:
                                    <input id="datetimePickerBegin" class="Wdate" style="height: 24px; border-color: #c5c5c5;" type="text" onclick="WdatePicker({ el: this, dateFmt: 'yyyy-MM-dd HH:mm:ss' })">
                                        到                                    
                                        <input id="datetimePickerEnd" class="Wdate" style="height: 24px; border-color: #c5c5c5;" type="text" onclick="WdatePicker({ el: this, dateFmt: 'yyyy-MM-dd HH:mm:ss' })">
                                    </div>
    ...
    <script type="text/javascript">  
        ...
        function Search() {
            ...
            var ProductionSheet = $("#txtProductionSheet").val().replace('/\,/g', '，').replace('/\:/g', '：').replace('/\;/g', '；');
            var StationName = $("#sStationName").val();
            var BeginTime = $("#datetimePickerBegin").val();
            var EndTime = $("#datetimePickerEnd").val();
            var Status = $("#SelectStatus").val();
            datagridfresh(1, 10, ERPDetailCode, EndProductSN, ProductionId, ProductionName, ProductionSheet, StationName, BeginTime, EndTime,Status);
        }
        function getFItemDetail(index, rowmaster) {
            var ddv = $('#datawin').datagrid('getRowDetail', index).find('table.ddv');
            ddv.datagrid({
                url: "/Controller/EndProductSeach.ashx?Id=" + rowmaster.ID,
                fitColumns: true,
                rownumbers: true,
                loadMsg: '加载中......',
                height: 'auto',
                ...
                columns: [[
                    ...
                    { field: 'ProductionSheet', width: 120, title: '订单编号', align: 'center' },
                    {
                        field: 'Status', width: 80, align: 'center', title: '状态',
                        formatter: function (value, row, index) {
                            switch (row.Status) {
                                case 1: return "生产"; break;
                                case 2: return "下线"; break;
                            }
                        }
                    },
                    { field: 'UpdateTime', width: 130, align: 'center', title: '最后更新时间' },
                    { field: 'ERPDetailCode', width: 100, align: 'center', title: '订单号' },
                    { field: 'ERPDetailCode', width: 80, align: 'center', title: '订单顺序号' }
                ]],
        ...
        function datagridfresh(pageNumber, pageSize, ERPDetailCode, EndProductSN, ProductionId, ProductionName, ProductionSheet, StationName, BeginTime, EndTime,Status) {
            $("#datawin").datagrid("loading");
            $.post('/Controller/EndProductionList.ashx',
                {
                    pageindex: pageNumber,
                    pagesize: pageSize,
                    erpDetailCode: ERPDetailCode,
                    ...
                    if (data.rows[0].tips != "没有数据") {
                        ...
                        $("#datawin").datagrid({
                            ...
                            columns: [[
                                {
                                    field: 'ID', width: 100, align: 'center', title: '操作',
                                    formatter: function (value, row, index) {
                                        if (row.Status == 1) {
                                            return '<a href="#" onclick=\'javascript:Cancle("' + row.ID
                                                + '","' + row.EndProductSN
                                                + '");\'>报废</a>';
                                        }
                                        else {
                        ...
                        $('#datapager').pagination({
                            ...
                            onSelectPage: function (pageNumber, pageSize) {
                                ...
                                datagridfresh(pageNumber, pageSize, ERPDetailCode, EndProductSN, ProductionId, ProductionName, ProductionSheet, StationName, BeginTime, EndTime);
                            }
                        });
                    }
                    else {
        ...
        function Cancle(ID, EndProductSN) {
            if (confirm("确定报废产品吗？")) {
                    $.post('/Controller/EndProductCancel.ashx', { id: ID,endProductSN:EndProductSN}, function (data) {
                        if (data == "0") {
                            $("#msg").html("报废失败！");
                        }
                        else {
