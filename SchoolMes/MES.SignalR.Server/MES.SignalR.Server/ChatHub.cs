    ...
    public class ChatHub : Hub
    {
        ...
        public void Register(string userName)
        {
            ...
            if (OnlineUsers.Count(x => x.ConnectionId == connnectId) == 0)
            {
                ...
                IOHelper.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + string.Format("User {0} on line now", userName));
            }
        ...
        public void SendPrivateMessage(string toUserName, string message)
        {
            ...
            var toUser = OnlineUsers.FindAll(x => x.UserName == toUserName);
            var fromUser = OnlineUsers.FirstOrDefault(x => x.ConnectionId == fromConnectionId);

            foreach (UserInfo item in toUser)
            {
                if (item != null)
                {
                    Clients.Client(item.ConnectionId).receivePrivateMessage(fromUser.UserName, message);
                    Clients.Client(item.ConnectionId).receivePrivateMessage(message);
                    IOHelper.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + string.Format("send massage '{0}',from {1} to {2}", message, fromUser.UserName, toUserName));
                }
                else
                {
                    //表示对方不在线
                    Clients.Caller.absentSubscriber();
                    //Clients.Caller.addMessage(fromUser.UserName, toUserName+"downline!");
                    IOHelper.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + string.Format("{0} is Off line now", toUserName));
                }
            }
        }
        ...
        public override Task OnConnected()
        {
            ...
            IOHelper.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + string.Format("Client On line，ID:{0},Total On Line {1}", Context.ConnectionId, OnlineUsers.Count + 1));
        ...
        public override Task OnDisconnected(bool stopCalled)
        {
            ...
            IOHelper.WriteLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + string.Format("Client Off line,ID:{0},UserNmae is {2},Total On Line {1}", Context.ConnectionId, OnlineUsers.Count, user.UserName));
            return base.OnDisconnected(stopCalled);
        }
